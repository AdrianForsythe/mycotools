...
def jgi_redundancy_check(db, jgi_df, duplicates={}, ome_col="portal", jgi2ncbi={}):
    ...
    # Replace NaN values in the 'version' column with '0.0'
    jgi_df.loc[:, "version"] = jgi_df["version"].replace(np.nan, "0.0")
    ...
    if ome not in ome_set:
        ...
        for typ in dwnlds:
            ...
            if not isinstance(check, int):
                # Ensure the '_path' column exists, if not, create it
                column_name = new_typ + "_path"
                if column_name not in jgi_df.columns:
                    jgi_df[column_name] = (
                        ""  # Initialize the column if it doesn't exist
                    )

                # Explicitly set the path value
                jgi_df.at[i, column_name] = check  # Safe with .at after copy
                ...
                if typ in {"gff3", "fna"}:
                    ...
                    # Ensure 'version' type compatibility in failed list
                    failed.append([ome, str(jgi_df["version"][i])])
                    jgi_df = jgi_df.drop(index=[i])  # Use explicit index drop
                    if ran_dwnld:
                        time.sleep(60)
                    break
            if ran_dwnld:
