...
def dwnld_mycocosm(
    ...
    if not os.path.isfile(out_file):
        for attempt in range(3):
            curl_cmd = subprocess.call(
                ["curl", mycocosm_url, "-o", out_file + ".tmp"], stdout=subprocess.PIPE
            )
            if not curl_cmd:
    ...
    jgi_df = pd.read_csv(out_file, encoding=encoding)
...
def prep_taxa_cols(df, col="#Organism/Name"):
    # Ensure columns exist and are of the appropriate dtype
    df["genus"] = pd.Series("", dtype="object")
    df["species"] = pd.Series("", dtype="object")
    df["strain"] = pd.Series("", dtype="object")

    for i, row in df.iterrows():
        organism = re.sub(r"[^ a-zA-Z0-9]", "", row[col]).split()
...
def prep_jgi_cols(jgi_df, name_col="name"):
    # Initialize required columns with appropriate types
    jgi_df = jgi_df.assign(
        publication_s=jgi_df["publication(s)"].fillna("").astype(str),
        genus="",
        species="",
        strain="",
        version=0.0,
    ...
    for i, row in jgi_df.iterrows():
        # Extract genus and species from `name_col`
        taxa = re.sub(r"[^ a-zA-Z0-9\.]", "", row[name_col]).split()
        jgi_df.at[i, "genus"] = taxa[0].replace(".", "")

        # Ensure the 'publication(s)' column is a string before calling .replace()
        publication = str(jgi_df.at[i, "publication(s)"])  # Ensure it's a string
        jgi_df.at[i, "publication(s)"] = publication.replace('"""', "")

        # Handle species, strain, and version parsing
        if len(taxa) > 1:
            jgi_df.at[i, "species"] = taxa[1]

            if len(taxa) > 2:
                strain = "".join(taxa[2:])
                vers_search = re.search(r"v(\d+\.\d+$)", strain)

                # Set version if found and update strain accordingly
                if vers_search:
                    jgi_df.at[i, "version"] = float(vers_search[1])
                    strain = strain[: -len(vers_search[0])]

                jgi_df.at[i, "strain"] = strain.replace(".", "")
        else:
            jgi_df.at[i, "species"] = "sp."

    # Sort by `version` in descending order and drop duplicates based on 'portal'
    jgi_df = jgi_df.sort_values(by="version", ascending=False).drop_duplicates("portal")
...
def ref_update(
    ...
    if taxonomy:  # already completed
        tax_dicts = gather_taxonomy(new_mtdb, api_key=ncbi_api, king=kingdom, rank=rank)
...
def extract_constraint_lineages(df, ncbi_api, kingdom, lineage_constraints, tax_dicts):
    ...
    tax_dicts = gather_taxonomy(
        df, api_key=ncbi_api, king=kingdom, rank=query_rank, tax_dicts=tax_dicts
    )
...
def taxonomy_update(
    ...
    tax_dicts = gather_taxonomy(taxless_db, api_key=ncbi_api, king=group, rank=rank)
...
def rogue_update(
    ...
    pre_ncbi_df0 = dwnld_ncbi_table(update_path + date + ".ncbi.tsv", group=group)
    ...
    tax_dicts = {
        v["genus"]: v["taxonomy"]
        for k, v in db.iterrows()
        if any(
            y
            for x, y in v["taxonomy"].items()
    ...
    if lineage_constraints:
        ...
        if not os.path.isfile(lineage_path):
            ...
            tax_dicts, ncbi_df = extract_constraint_lineages(
                ncbi_df, ncbi_api, kingdom, lineage_constraints, tax_dicts
            )
    ...
    if jgi:
        ...
        if lineage_constraints:
            ...
            if not os.path.isfile(lineage_path):
                ...
                tax_dicts, jgi_df = extract_constraint_lineages(
                    jgi_df, ncbi_api, kingdom, lineage_constraints, tax_dicts
                )
        ...
        if os.path.isfile(jgi_ncbi_overlap_file):
            ...
            ncbi_df, jgi2ncbi, jgi2biosample, true_ncbi, ncbi_jgi_overlap, todel_i = (
                rm_ncbi_overlap(ncbi_df, jgi_df, jgi2ncbi, true_ncbi, api=api)
            )
...
def control_flow(
    ...
    if add or predb:  # add predb2mtdb 2 master database
        ...
        tax_dicts = gather_taxonomy(addDB, api_key=ncbi_api, king=king, rank=rank)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Initializes or updates " + "MycotoolsDB (MTDB)"
    )
    ...
    init_args.add_argument(
        "--predb", help="[-i]: Initialize primary MTDB using a reference predb .tsv"
    )
    ...
    upd_args.add_argument(
        "-t", "--taxonomy", action="store_true", help="Update taxonomy and exit"
    )
    upd_args.add_argument(
        "-k",
        "--keep",
