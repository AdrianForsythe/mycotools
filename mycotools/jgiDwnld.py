...
def check_md5(file_path, expected_md5):
    """Check MD5 hash of the downloaded file against the expected MD5."""
    md5_cmd = subprocess.run(
        ["md5sum", file_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    md5_res = md5_cmd.stdout.decode("utf-8")
    md5_find = re.search(r"\w+", md5_res)
    return md5_find[0] if md5_find else None

@retry(stop_max_attempt_number=3, wait_fixed=60000)  # 3 attempts with 1-minute wait
def jgi_dwnld(ome, file_type, output, masked=True, spacer="\t"):
    ...
    preexisting, check, ran_dwnld, org_name = False, 1, False, None
    ...
    if os.path.exists(dwnld):
        if dwnld_md5 and check_md5(dwnld, dwnld_md5) == dwnld_md5:
            return dwnld, True, file_type, False, org_name
        else:
            os.remove(dwnld)  # Remove the corrupted file
    ...
    for attempt in range(1, 4):
        ...
        if curl_cmd == 0:
            if dwnld_md5 and check_md5(dwnld, dwnld_md5) == dwnld_md5:
                return dwnld, False, file_type, ran_dwnld, org_name

            elif not dwnld_md5:
                if os.path.getsize(dwnld) > 1024:  # File size check as a fallback
                    return dwnld, False, file_type, ran_dwnld, org_name

        print(f"{spacer} Attempt {attempt}: MD5 mismatch or download error", flush=True)
        time.sleep(60)  # Wait before retry

    # After retries are exhausted
    print(spacer + "\tERROR: Excluding from database - potential failure", flush=True)
    return 2, preexisting, file_type, ran_dwnld, org_name
